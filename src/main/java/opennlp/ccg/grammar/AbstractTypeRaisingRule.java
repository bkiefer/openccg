...
public abstract class AbstractTypeRaisingRule extends AbstractRule {
    ...
    /* Not sure this is right, so i rather comment it out (bk) */
    public Category getArg() {
      return _arg;
    }

    /** Returns the result category of this rule */
    public Category getResult() {
      Category result = _result.copy();
      ComplexCat range;
      if (_result instanceof ComplexCat) {
        range = (ComplexCat) _result.copy();
        range.add(new BasicArg(_embeddedSlash, _arg));
        ((ComplexCat)result).add(new BasicArg(_upperSlash, range));
      } else {
        range = new ComplexCat((TargetCat)_result.copy(),
            new BasicArg(_embeddedSlash, _arg));
        result = new ComplexCat((TargetCat)_result.copy(),
